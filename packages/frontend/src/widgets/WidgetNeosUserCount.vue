	...
	<MkContainer :show-header="widgetProps.showHeader">
		...
		<div :class="$style.root">
			<div v-if="loaded" :class="$style.flex">
				...
				<div>
					<p><i class="ti ti-cardboards" style="color: #D76FF9"></i>&nbsp;{{userCountData.resonite_vr}}</p>
					<p><i class="ti ti-device-desktop" style="color: #6FED51"></i>&nbsp;{{userCountData.resonite_screen}}</p>
					<p><i class="ti ti-server" style="color: hsl(-180, 80%, 70%)"></i>&nbsp;{{userCountData.resonite_headless}}</p>
				</div>
			</div>
		</div>
	</MkContainer>
...
type NeosUserCount = {
  resonite_headless: number
  resonite_mobile: number
  resonite_all: number
  resonite_screen: number
  resonite_vr: number
}

const userCountData = ref<NeosUserCount>({resonite_vr: 0, resonite_headless: 0, resonite_screen: 0, resonite_all:0, resonite_mobile:0});
const loaded = ref<boolean>(false);

const api = "https://statistics.kokoa.dev/v1/resonite/usercount";
